...
DATA_DIR = os.environ.get('CHIMERA_DATA_DIR', os.path.join(PROJECT_ROOT, 'chimera_data'))
PRIMARY_TRADELOG = os.path.join(PROJECT_ROOT, 'data', 'tradelog_chimera_fip.csv')
REGIME_TRACE_PATH = os.path.join(PROJECT_ROOT, 'data', 'regime_trace_chimera_fip.csv')
WEEKLY_TRACE_PATH = os.path.join(PROJECT_ROOT, 'data', 'weekly_returns_chimera_fip.csv')
LEGACY_TRADELOGS = [
...
class ChimeraEngineNormal:
    ...
    def _classify_regime(self, idx: int, broad, risk, fear, usd, us10y, gold, breadth: float) -> Tuple[str, float, float, float, str, float]:
        ...
        if any(pd.isna(x) for x in [sma200, eff, slope, momz]):
            return 'CHOP', breadth, vix, macro_score, 'Insufficient history', 0.0
        ...
        regime_score = 0.50 * trend_score + 0.20 * breadth_score + 0.20 * risk_score + 0.10 * np.tanh(macro_score * 10.0)
        confidence = float(np.clip(abs(regime_score) / 2.5, 0.0, 1.0))

        # BULL: Nifty above SMA200 is the primary gate (matching 71.7% of actual history)
        if above_sma > 0 and regime_score > 0.20 and breadth >= CONFIG['BREADTH_BULL']:
            return 'BULL', breadth, vix, macro_score, 'Nifty above SMA200 + breadth OK', confidence
        if above_sma > 0 and regime_score > -0.30:
            return 'CHOP', breadth, vix, macro_score, 'Nifty above SMA200 but mixed signals', confidence
        # BEAR: Only when Nifty BELOW SMA200 AND stress confirmation
        if above_sma < 0 and (breadth < CONFIG['BREADTH_BEAR'] or (np.isfinite(vix) and vix >= CONFIG['VIX_HIGH'])):
            return 'BEAR', breadth, vix, macro_score, 'Nifty below SMA200 + stress confirmation', confidence
        if above_sma < 0:
            return 'CHOP', breadth, vix, macro_score, 'Nifty below SMA200 but no stress', confidence
        return 'CHOP', breadth, vix, macro_score, 'Mixed signals / low conviction', confidence
    ...
    def run_simulation(self):
        ...
        for rebalance_idx in range(start_rebalance_idx, len(rebalance_dates) - 1):
            ...
            regime, breadth, vix, macro_score, regime_reason, confidence = self._classify_regime(signal_idx, broad, risk, fear, usd, us10y, gold, breadth)
            ...
            if scored.empty:
                self.trade_log.append({
                    ...
                    'market_state': regime, 'regime_confidence': confidence, 'regime_score': np.nan,
                    ...
                    'mom_z': np.nan, 'break_score': np.nan,
                })
                continue
