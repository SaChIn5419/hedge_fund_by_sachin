...
class PolarsTearSheet:
    ...
    def get_top_trades(self, trades_df, n=20):
        ...
        # Add trade duration (days)
        # Trades log has 'date', but to get duration we need the entry date, which isn't directly in this CSV format.
        # Alternatively, we calculate frequency and assume 1 trade = 1 period (week).
        # We will add it to get_trade_frequency instead.
    ...
    def get_trade_frequency(self, trades_df):
        ...
        freq = trades_df.group_by("ticker").agg([
            ...
            (pl.count() * 7).alias("tot_days"), # Assuming weekly rebalance roughly 7 days per trade
    ...
    def generate_v10_dashboard(self, df_pandas, bench_series, trades_pd=None):
        ...
        fig.add_trace(go.Scatter(x=combined.index, y=combined['weight'], name='Gross Leverage',
    ...
    def generate(self, df: pl.DataFrame, trades_df: pl.DataFrame = None, open_browser: bool = True, output_file: str = "architect_tearsheet_v10.html"):
        ...
        sharpe = (np.mean(rets) * annual_factor - self.rf) / vol_ann if vol_ann > 0 else 0
        
        downside_rets = rets[rets < 0]
        downside_std = np.std(downside_rets) * np.sqrt(annual_factor)
        sortino = (np.mean(rets) * annual_factor - self.rf) / downside_std if downside_std > 0 else 0
        ...
        if trades_df is not None:
            ...
            # Average Leverage
            avg_leverage = t_df.groupby('date')['weight'].sum().mean()
            max_leverage = t_df.groupby('date')['weight'].sum().max()
