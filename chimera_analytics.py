...
class PolarsTearSheet:
    ...
    def get_drawdown_table(self, df):
        """Identifies the worst drawdowns and only marks recovery when recovered."""
        pdf = df.to_pandas().copy()
        pdf["date"] = pd.to_datetime(pdf["date"])
        pdf = pdf.sort_values("date").reset_index(drop=True)
        pdf["peak"] = pdf["equity"].cummax()
        pdf["drawdown"] = pdf["equity"] / pdf["peak"] - 1.0

        peaks = pdf.index[pdf["equity"] == pdf["peak"]].tolist()
        if not peaks:
            return []

        records = []
        for i, start_idx in enumerate(peaks):
            end_idx = peaks[i + 1] if i + 1 < len(peaks) else len(pdf) - 1
            seg = pdf.iloc[start_idx : end_idx + 1].copy()
            if seg.empty or seg["drawdown"].min() >= 0:
                continue
            trough_idx = seg["drawdown"].idxmin()
            peak_val = float(pdf.loc[start_idx, "equity"])
            after_trough = pdf.iloc[trough_idx + 1 :]
            recovered = after_trough.index[after_trough["equity"] >= peak_val]
            recovered_date = pdf.loc[recovered[0], "date"] if len(recovered) else pd.NaT
            records.append(
                {
                    "drawdown": float(seg["drawdown"].min()),
                    "started": pdf.loc[start_idx, "date"],
                    "recovered": recovered_date,
                    "days": int((trough_idx - start_idx) + (0 if pd.isna(recovered_date) else (recovered[0] - trough_idx))),
                }
            )

        if not records:
            return []
        out = pd.DataFrame(records).sort_values("drawdown").head(10)
        out["started"] = pd.to_datetime(out["started"]).dt.strftime("%Y-%m-%d")
        out["recovered"] = pd.to_datetime(out["recovered"]).dt.strftime("%Y-%m-%d")
        out["recovered"] = out["recovered"].fillna("N/A")
    ...
    def get_eoy_table(self, df):
        ...
        eoy = (
            ...
            .group_by("Year")
            .agg([(((1 + pl.col("ret")).product()) - 1).alias("Strategy")])
            .sort("Year")
        )
        return eoy.to_dicts()

    def get_top_trades(self, trades_df, n=20):
        """Get top N profitable and worst N active trades."""
        if trades_df is None or trades_df.height == 0:
            return [], []

        trades_df = trades_df.with_columns(pl.col("date").cast(pl.Date))
        active = trades_df.filter(pl.col("weight").abs() > 1e-12)
        if active.height == 0:
            return [], []

        top_winners = (
            active.sort("net_pnl", descending=True)
            .head(n)
            .select(["date", "ticker", "close", "net_pnl"])
    ...
    def get_trade_frequency(self, trades_df):
        """Get active trade count by ticker."""
        if trades_df is None or trades_df.height == 0:
            return []
        active = trades_df.filter(pl.col("weight").abs() > 1e-12).with_columns(pl.col("date").cast(pl.Date))
        if active.height == 0:
            return []
        freq = (
            active.group_by("ticker")
            .agg(
                [
    ...
    def generate_v10_dashboard(self, df_pandas, bench_series, trades_pd=None):
        ...
        if bench_series is None:
            bench_series = pd.Series(
                df_pandas["equity"].to_numpy(),
                index=pd.to_datetime(df_pandas["date"]),
                name="benchmark_close",
            )
        elif not isinstance(bench_series, pd.Series):
            bench_series = pd.Series(bench_series)
            bench_series.name = "benchmark_close"
        ...
        combined["benchmark_equity"] = combined["benchmark_close"] * (
            initial_equity / start_price
        )
        combined["bench_returns"] = combined["benchmark_close"].pct_change().fillna(0)
        combined["bench_drawdown"] = (
            combined["benchmark_equity"] - combined["benchmark_equity"].cummax()
        ) / combined["benchmark_equity"].cummax()
        ...
        # Rolling Alpha
        combined["rolling_alpha"] = combined["ret"] - (
            combined["rolling_beta"] * combined["bench_returns"]
        )
        combined["rolling_alpha_ann"] = (
            combined["rolling_alpha"].rolling(window).mean() * annual_factor
        )
        ...
        if trades_pd is not None:
            ...
            trades_pd = trades_pd.copy()
            trades_pd["gross_weight"] = trades_pd["weight"].abs()
            daily_trades = (
                trades_pd.groupby("date")
                .agg(
                    {
                        "kinetic_energy": "mean",
                        "leverage_mult": "mean",
            ...
            daily_trades = daily_trades.rename(columns={"gross_weight": "weight"})
        ...
        # ROW 2: Physics State (Energy) - REMOVED
        ...
        fig.add_trace(
            go.Scatter(
                x=combined.index,
                y=combined["nifty_vol"],
                name="Nifty Vol",
                line=dict(color="#ff5555", dash="dot", width=1.5),
            ),
            row=3,
            col=1,
            secondary_y=True,
        )
        fig.update_yaxes(title="Leverage", row=3, col=1, gridcolor="#222")
        ...
        fig.update_yaxes(
            title="Exposure", row=4, col=1, gridcolor="#222", range=[0, 2.0]
        )
        ...
        fig.add_trace(
            go.Scatter(
                x=combined.index,
                y=combined["rolling_alpha_ann"],
                name="Ann. Alpha",
                line=dict(color="#00ffcc", width=1.5, dash="solid"),
            ),
            row=5,
            col=1,
        )  # Same axis for simplicity or dual?
    ...
    def generate(
        ...
        output_file: str = "architect_tearsheet_v10.html",
    ):
        ...
        df = (
            ...
            .with_columns([(pl.col("equity") / pl.col("peak") - 1).alias("drawdown")])
            .drop_nulls()
        )
        ...
        # Determine actual trading duration in years using calendar elapsed time
        start_date = df["date"].iloc[0]
        end_date = df["date"].iloc[-1]
        calendar_days = (end_date - start_date).days
        years = calendar_days / 365.25 if calendar_days > 0 else 1

        cagr = (final_equity / initial_capital) ** (1 / years) - 1

        # Auto-detect frequency for annualized metrics
        avg_gap = calendar_days / df.height if df.height > 1 else 1
        annual_factor = 252 if avg_gap <= 4 else 52
        ...
        # Risk Ratios
        vol_ann = np.std(rets) * np.sqrt(annual_factor)
        sharpe = (
            (np.mean(rets) * annual_factor - self.rf) / vol_ann if vol_ann > 0 else 0
        )

        downside_rets = rets[rets < 0]
        downside_std = np.std(downside_rets) * np.sqrt(annual_factor)
        sortino = (
            (np.mean(rets) * annual_factor - self.rf) / downside_std
            if downside_std > 0
            else 0
        )
        ...
        # Trading Physics
        periods_won = np.sum(rets > 0)
        periods_lost = np.sum(rets < 0)
        win_rate = periods_won / df.height if df.height > 0 else 0

        avg_win = np.mean(rets[rets > 0]) if periods_won > 0 else 0
        avg_loss = np.mean(rets[rets < 0]) if periods_lost > 0 else 0
        win_loss_ratio = abs(avg_win / avg_loss) if avg_loss != 0 else 0

        # Trade Frequency Metrics
        total_trades = int((trades_df.to_pandas()["weight"].abs() > 1e-12).sum()) if trades_df is not None else 0
        ...
        if trades_df is not None:
            ...
            t_df["gross_weight"] = t_df["weight"].abs()
            active_t_df = t_df[t_df["gross_weight"] > 1e-12].copy()
            if "universe_size" in active_t_df.columns:
                universe_size = int(active_t_df["universe_size"].max())

            # Market State Attribution
            if not active_t_df.empty:
                state_stats = (
                    active_t_df.groupby("market_state")
                    .agg(
                        {
                            "net_pnl": "sum",
                            "kinetic_energy": "mean",
                            "weight": "count",  # Active trade count
                        }
                    )
                    .rename(columns={"weight": "trades"})
                    .reset_index()
                )
            else:
            ...
            # Structure Monitor
            # Hit rate: Positive PnL / Total active trades
            if not active_t_df.empty:
                active_t_df["is_win"] = active_t_df["net_pnl"] > 0
                if "structure_tag" in active_t_df.columns:
                    struct_stats = (
                        active_t_df.groupby("structure_tag")
                        .agg({"net_pnl": "sum", "is_win": "mean", "weight": "count"})
                        .rename(columns={"is_win": "hit_rate", "weight": "trades"})
                        .reset_index()
                    )
                else:
            ...
            daily_gross = active_t_df.groupby("date")["gross_weight"].sum() if not active_t_df.empty else pd.Series(dtype=float)
            turnover = daily_gross.diff().abs().mean() if len(daily_gross) > 1 else 0.0

            avg_leverage = daily_gross.mean() if len(daily_gross) else 0.0
            max_leverage = daily_gross.max() if len(daily_gross) else 0.0
        ...
        plotly_html = self.generate_v10_dashboard(
            df_pd, bench, trades_df.to_pandas() if trades_df is not None else None
        )
            ...
            <div class="container">
                ...
                <div class="metrics-grid">
                    ...
                    <div class="metric-box">
                        ...
                        <div class="kv-row"><span class="kv-key">Universe Size</span><span class="kv-val">{universe_size}</span></div>
                        ...
                        <div class="kv-row"><span class="kv-key">Win Rate (Weeks)</span><span class="kv-val">{win_rate * 100:.2f}%</span></div>
                    ...
                    <div class="metric-box">
                        ...
                        <div class="kv-row"><span class="kv-key">Avg. Leverage</span><span class="kv-val">{avg_leverage:.2f}x</span></div>
                        <div class="kv-row"><span class="kv-key">Max. Leverage</span><span class="kv-val">{max_leverage:.2f}x</span></div>
                        ...
                        <div class="kv-row"><span class="kv-key">Skew / Kurt</span><span class="kv-val">{skewness:.2f} / {kurtosis:.2f}</span></div>
                    </div>
                ...
                <div class="tables-grid">
                    <div>
                         <h2>Stock Holdings Analysis</h2>
                         <table>
                            <thead><tr><th>Ticker</th><th>Pns</th><th>Est. Days</th><th>Net PnL</th></tr></thead>
                            <tbody>
                                {"".join([f"<tr><td>{r['ticker']}</td><td>{r['trades']}</td><td>{r['tot_days']}</td><td class='{'pos' if r['total_pnl'] > 0 else 'neg'}'>₹{r['total_pnl']:,.0f}</td></tr>" for r in trade_freq]) if trade_freq else ""}
                            </tbody>
                        </table>
                    </div>
                </div>
        ...
        print(f"SUCCESS: V10 Tearsheet saved as '{output_file}'.")
        if open_browser:
